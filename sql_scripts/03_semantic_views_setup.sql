...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.CLINICAL_ANALYTICS_VIEW
    ...
    relationships (
        ENCOUNTERS.PATIENT_KEY = PATIENTS.PATIENT_KEY,
        ENCOUNTERS.PROVIDER_KEY = PROVIDERS.PROVIDER_KEY,
        ENCOUNTERS.DEPARTMENT_KEY = DEPARTMENTS.DEPARTMENT_KEY,
        ENCOUNTERS.PRIMARY_DIAGNOSIS_KEY = DIAGNOSES.DIAGNOSIS_KEY,
        CLINICAL_MEASURES.PATIENT_KEY = PATIENTS.PATIENT_KEY,
        CLINICAL_MEASURES.ENCOUNTER_KEY = ENCOUNTERS.ENCOUNTER_KEY,
        MEDICATIONS.PATIENT_KEY = PATIENTS.PATIENT_KEY,
        MEDICATIONS.ENCOUNTER_KEY = ENCOUNTERS.ENCOUNTER_KEY
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.OPERATIONAL_ANALYTICS_VIEW
    ...
    relationships (
        OPERATIONS.DEPARTMENT_KEY = DEPARTMENTS.DEPARTMENT_KEY,
        QUALITY_METRICS.DEPARTMENT_KEY = DEPARTMENTS.DEPARTMENT_KEY
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.RESEARCH_ANALYTICS_VIEW
    ...
    relationships (
        RESEARCH_OUTCOMES.STUDY_KEY = STUDIES.STUDY_KEY,
        RESEARCH_OUTCOMES.PATIENT_KEY = PATIENTS.PATIENT_KEY,
        RESEARCH_OUTCOMES.PROVIDER_KEY = PROVIDERS.PROVIDER_KEY
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.FINANCIAL_ANALYTICS_VIEW
    ...
    relationships (
        FINANCIAL_TRANSACTIONS.ENCOUNTER_KEY = ENCOUNTERS.ENCOUNTER_KEY,
        FINANCIAL_TRANSACTIONS.PAYER_KEY = PAYERS.PAYER_KEY,
        FINANCIAL_TRANSACTIONS.DEPARTMENT_KEY = DEPARTMENTS.DEPARTMENT_KEY,
        ENCOUNTERS.PATIENT_KEY = PATIENTS.PATIENT_KEY
    )
    facts (
