...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.CLINICAL_ANALYTICS_VIEW
    tables (
        PATIENT_ENCOUNTER_FACT as ENCOUNTERS,
        PATIENT_DIM as PATIENTS,
        PROVIDER_DIM as PROVIDERS,
        DEPARTMENT_DIM as DEPARTMENTS,
        DIAGNOSIS_DIM as DIAGNOSES,
        CLINICAL_MEASURES_FACT as CLINICAL_MEASURES,
        MEDICATION_FACT as MEDICATIONS
    )
    relationships (
        ENCOUNTERS(PATIENT_KEY) references PATIENTS(PATIENT_KEY),
        ENCOUNTERS(PROVIDER_KEY) references PROVIDERS(PROVIDER_KEY),
        ENCOUNTERS(DEPARTMENT_KEY) references DEPARTMENTS(DEPARTMENT_KEY),
        ENCOUNTERS(PRIMARY_DIAGNOSIS_KEY) references DIAGNOSES(DIAGNOSIS_KEY),
        CLINICAL_MEASURES(PATIENT_KEY) references PATIENTS(PATIENT_KEY),
        CLINICAL_MEASURES(ENCOUNTER_KEY) references ENCOUNTERS(ENCOUNTER_KEY),
        MEDICATIONS(PATIENT_KEY) references PATIENTS(PATIENT_KEY),
        MEDICATIONS(ENCOUNTER_KEY) references ENCOUNTERS(ENCOUNTER_KEY)
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.OPERATIONAL_ANALYTICS_VIEW
    tables (
        OPERATIONAL_FACT as OPERATIONS,
        DEPARTMENT_DIM as DEPARTMENTS,
        QUALITY_METRICS_FACT as QUALITY_METRICS
    )
    relationships (
        OPERATIONS(DEPARTMENT_KEY) references DEPARTMENTS(DEPARTMENT_KEY),
        QUALITY_METRICS(DEPARTMENT_KEY) references DEPARTMENTS(DEPARTMENT_KEY)
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.RESEARCH_ANALYTICS_VIEW
    tables (
        RESEARCH_OUTCOMES_FACT as RESEARCH_OUTCOMES,
        RESEARCH_STUDY_DIM as STUDIES,
        PATIENT_DIM as PATIENTS,
        PROVIDER_DIM as PROVIDERS
    )
    relationships (
        RESEARCH_OUTCOMES(STUDY_KEY) references STUDIES(STUDY_KEY),
        RESEARCH_OUTCOMES(PATIENT_KEY) references PATIENTS(PATIENT_KEY),
        RESEARCH_OUTCOMES(PROVIDER_KEY) references PROVIDERS(PROVIDER_KEY)
    )
    facts (
...
CREATE OR REPLACE SEMANTIC VIEW PEDIATRIC_HOSPITAL_AI_DEMO.CLINICAL_SCHEMA.FINANCIAL_ANALYTICS_VIEW
    tables (
        FINANCIAL_FACT as FINANCIAL_TRANSACTIONS,
        PATIENT_ENCOUNTER_FACT as ENCOUNTERS,
        PATIENT_DIM as PATIENTS,
        PAYER_DIM as PAYERS,
        DEPARTMENT_DIM as DEPARTMENTS
    ...
    relationships (
        FINANCIAL_TRANSACTIONS(ENCOUNTER_KEY) references ENCOUNTERS(ENCOUNTER_KEY),
        FINANCIAL_TRANSACTIONS(PAYER_KEY) references PAYERS(PAYER_KEY),
        FINANCIAL_TRANSACTIONS(DEPARTMENT_KEY) references DEPARTMENTS(DEPARTMENT_KEY),
        ENCOUNTERS(PATIENT_KEY) references PATIENTS(PATIENT_KEY)
    )
    facts (
